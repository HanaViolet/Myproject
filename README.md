...
- **主题**：功能扩展与优化
- **学习内容**：
  - 在第十五周框架基础上，完善数据库的增删改查操作
  - 增加用户权限管理和数据导出功能
  - 优化命令行工具的交互体验
- **收获**：
  - 实现了完整的增删改查功能，例如：
    ```sql
    INSERT INTO Courses (course_id, course_name) VALUES (101, 'Database Systems');
    DELETE FROM Students WHERE student_id = 1002;
    ```
  - 增加了部分用户便捷操作，细节不便公开
  - 开发了数据导出功能，支持备份到CSV：
    ```sql
    COPY (SELECT * FROM Students) TO '/data/students_backup.csv' DELIMITER ',' CSV HEADER;
    ```
  - 增加了基于我自己理解的错误提示，便捷用户修改操作
- **反思**：
  - 权限管理让我对数据库的访问控制有了更深理解
  - 数据导出功能调试时路径问题让我头疼，得多注意环境配置
  - 优化交互界面后，用户体验好多了，很有成就感
- **注意事项**：
  - 权限分配要明确，避免给用户过多权限
  - 导出文件路径需检查权限和格式，防止导出失败
  - 增删改查操作需加入错误处理，提高工具鲁棒性

---
## 总结

这16周的数据库课程让我从一头雾水到能独立设计数据库，收获满满。SQL模板和示例让我对每个知识点有了更直观的理解，反思和注意事项则记录了我的踩坑经历和成长。学数据库的过程像解谜，既烧脑又有趣，尤其是SQL注入和索引优化让我特别着迷。谨以这份记录致我的大二下期数据库学习！
